...
function LiveWhiteboard({ currentText, accent }: { currentText: string, accent: string }) {
  ...
  const displayText = currentText.length > 300 ? currentText.substring(currentText.length - 300) + "..." : currentText;
              ...
              <Canvas camera={{ position: [0, 1.2, 5.5], fov: 45 }}>
                 ...
                 {activePage === 'classroom' && (
                    <group>
                      ...
                      <LiveWhiteboard currentText={liveText} accent={accent} />
                    </group>
                 )}
                 {activePage === 'lab' && <MoleculeLab accent={accent} />}
                 {activePage === 'matrix' && (
                    <Float speed={4}>
          ...
          <div className="flex-[1.2] min-w-[400px] bg-slate-950/90 rounded-[8rem] border border-white/5 flex flex-col shadow-2xl overflow-hidden backdrop-blur-4xl relative group">
            <div className="p-10 border-b border-white/10 flex items-center justify-between">
               <div className={`flex items-center gap-4 font-black italic text-xs tracking-[0.4em] ${themeColors[accent].split(' ')[2]}`}>
                  <Eye size={20} className="animate-pulse" /> COMMAND HUB
               </div>
               <div className="flex items-center gap-4">
                 {isSpeaking && <VolumeX size={22} className="text-rose-500 cursor-pointer" onClick={() => {if(audioRef.current) audioRef.current.pause(); setIsSpeaking(false);}} />}
               </div>
            </div>
            ...
            <div className="p-10 pt-4 relative z-[150]">
              <div className="flex items-center gap-3 bg-slate-900/90 border border-white/10 p-2 rounded-[6rem]">
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Message..." className="flex-1 bg-transparent border-none py-4 px-2 text-sm text-white focus:outline-none placeholder:text-slate-700" />
